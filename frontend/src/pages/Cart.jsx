    ...
    <div className="min-h-screen bg-white">
                    ...
                    <td className="px-6 py-4">
                      <div className="flex items-center">
                        <div className="h-16 w-16 flex items-center justify-center mr-4">
                          <img src={product.imageUrl ? (product.imageUrl.startsWith('http') ? product.imageUrl : `http://localhost:3000${product.imageUrl}`) : "/src/assets/icon.png"} alt={product.name} className="h-16 w-16 rounded-lg" />
                        </div>
      ...
      <div className="flex flex-col lg:flex-row gap-8 justify-between max-w-7xl mx-auto px-4 sm:px-6 lg:px-8">
        ...
        <div className="w-full lg:w-1/3 border border-gray-200 rounded-lg p-6 bg-white shadow-sm">
          ...
          <button onClick={() => navigate('/checkout')} className="w-full mt-6 px-6 py-3 bg-red-600 hover:bg-red-700 text-white rounded-md transition-colors">
            Proceed to checkout
          </button>
        </div>
      </div>
